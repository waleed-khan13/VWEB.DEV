...
import { CheckCircle2, Palette, PenTool, Brush, Clapperboard } from "lucide-react";
...
function DesignContentSection({ section }: { section: Realm['sections'][0] }) {
    const { ref, isInView } = useInView();
    const Icon = sectionIcons[section.sectionTitle] || Palette;
                ...
                <div className="max-w-5xl mx-auto">
                    <div className="flex flex-col md:flex-row items-start gap-8 md:gap-12">
                        <div className="md:w-1/3 text-left">
                             <Icon className="w-12 h-12 text-yellow-500 mb-4 inline-block" />
                        ...
                        <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <Card key={item.title} className="bg-white/50 dark:bg-gray-900/30 border-accent/20">
                                <CardHeader>
...
export function DesignContent({ realm }: { realm: Realm }) {
    return (
        <div className="relative">
             <div className="relative z-10">
                {realm.sections.map((section) => (
                <DesignContentSection key={section.sectionTitle} section={section} />
                ))}
            </div>
        </div>
    );
