...
import { CheckCircle2, Component, Server, Smartphone, Cloud } from "lucide-react";
...
function BuildContentSection({ section }: { section: Realm['sections'][0] }) {
    const { ref, isInView } = useInView();
    const Icon = sectionIcons[section.sectionTitle] || Component;
                    ...
                    <div className="flex flex-col md:flex-row items-start gap-8 md:gap-12">
                        ...
                        <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {section.items.map((item) => (
                                <Card key={item.title} className="bg-gray-900/30 border-teal-500/20">
                                    <CardHeader>
...
export function BuildContent({ realm }: { realm: Realm }) {
    return (
        <>
            {realm.sections.map((section) => (
                <BuildContentSection key={section.sectionTitle} section={section} />
            ))}
        </>
    );
}
