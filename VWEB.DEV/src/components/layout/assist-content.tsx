...
function AssistContentSection({ section }: { section: Realm['sections'][0] }) {
    const { ref, isInView } = useInView();
    const Icon = sectionIcons[section.sectionTitle] || Headset;
                ...
                <div className="max-w-5xl mx-auto">
                     <div className="flex flex-col md:flex-row items-start gap-8 md:gap-12">
                        <div className="md:w-1/3 text-left">
                            <Icon className="w-12 h-12 text-rose-400 mb-4 inline-block" />
                        ...
                        <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {section.items.map((item) => (
                                <Card key={item.title} className="bg-gray-900/30 border-rose-500/20">
                                    <CardHeader>
...
export function AssistContent({ realm }: { realm: Realm }) {
    return (
        <div className="relative">
            <div className="relative z-10">
                {realm.sections.map((section) => (
                    <AssistContentSection key={section.sectionTitle} section={section} />
                ))}
            </div>
        </div>
    );
