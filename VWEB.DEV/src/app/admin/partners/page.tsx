...
export default function AdminPartnersPage() {
    ...
    if (isDialogOpen) {
        if (editingPartner) {
          setFormData({
            ...
            logoType: editingPartner.logo.type,
            logoContent: editingPartner.logo.content,
            imagePreview: editingPartner.logo.type === 'url' ? editingPartner.logo.content : null,
          });
        } else {
          setFormData(initialFormData);
        }
    }
  }, [isDialogOpen, editingPartner]);
  ...
  const handleSave = () => {
    ...
    if (editingPartner) {
      setPartners(partners.map(p => p.id === editingPartner.id ? { ...p, name: formData.name, logo: newLogo } : p));
    } else {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-headline text-3xl font-bold">Partners</h1>
          <p className="text-muted-foreground mt-2">
            Manage your official partners list.
          </p>
        </div>
          ...
          <DialogContent>
            ...
            <div className="grid gap-6 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="name" value={formData.name} onChange={handleInputChange} placeholder="Partner Name" className="col-span-3" />
              </div>
                   ...
                   <AlertDialog>
                      ...
                      <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                This action cannot be undone. This will permanently delete this partner from your list.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                              <AlertDialogCancel>Cancel</AlertDialogCancel>
                              <AlertDialogAction onClick={() => handleDelete(partner.id)} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">
                                Delete
                              </AlertDialogAction>
                          </AlertDialogFooter>
                      </AlertDialogContent>
